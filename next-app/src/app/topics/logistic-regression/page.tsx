...
import { logisticRegressionContent } from "../../../data/logistic-regression";
import PassFailPredictor from "../../../components/interactives/PassFailPredictor";
...
function InteractiveSection({ difficulty }: { difficulty: Difficulty }) {
  switch (difficulty) {
    case "beginner": return <PassFailPredictor />;
    case "intermediate": return <LogisticModelSim />;
    case "advanced": return <ROCExplorer />;
  }
}

export default function LogisticRegressionPage() {
  const [difficulty, setDifficulty] = useState<Difficulty>("beginner");
  const content = logisticRegressionContent[difficulty];
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-10">
        <div className="text-center space-y-4">
          <div className="w-28 h-28 rounded-2xl bg-white border border-brand-100 shadow-sm flex items-center justify-center mx-auto">
            <div className="scale-[2]"><TopicIcon name="logistic-regression" /></div>
          </div>
          <h1 className="font-serif text-4xl md:text-5xl text-brand-800">Logistic Regression</h1>
          <p className="text-brand-500 text-lg max-w-xl mx-auto">Predicting yes or no. The S-curve that powers classification.</p>
        </div>
        ...
        <section className="bg-white rounded-2xl border border-brand-100 shadow-sm p-6 md:p-8">
          ...
          <CustomExample topic="Logistic Regression" difficulty={difficulty} />
        </section>
      </main>
