...
import { linearRegressionContent } from "../../../data/linear-regression";
import LineBestFit from "../../../components/interactives/LineBestFit";
import RegressionDiagnostics from "../../../components/interactives/RegressionDiagnostics";
import MultipleRegression from "../../../components/interactives/MultipleRegression";
...
function InteractiveSection({ difficulty }: { difficulty: Difficulty }) {
  switch (difficulty) {
    case "beginner": return <LineBestFit />;
    case "intermediate": return <RegressionDiagnostics />;
    case "advanced": return <MultipleRegression />;
  }
}

export default function LinearRegressionPage() {
  const [difficulty, setDifficulty] = useState<Difficulty>("beginner");
  const content = linearRegressionContent[difficulty];
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-10">
        <div className="text-center space-y-4">
          <div className="w-28 h-28 rounded-2xl bg-white border border-brand-100 shadow-sm flex items-center justify-center mx-auto">
            <div className="scale-[2]"><TopicIcon name="linear-regression" /></div>
          </div>
          <h1 className="font-serif text-4xl md:text-5xl text-brand-800">Linear Regression</h1>
          <p className="text-brand-500 text-lg max-w-xl mx-auto">The workhorse of statistical modeling. Fit a line, make predictions.</p>
        </div>
        ...
        <section className="bg-white rounded-2xl border border-brand-100 shadow-sm p-6 md:p-8">
          ...
          <CustomExample topic="Linear Regression" difficulty={difficulty} />
        </section>
      </main>
